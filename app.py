...
def get_google_sheet_client():
    ...
    if gs_client is None and GOOGLE_SHEET_ENABLED:
        try:
            ...
            # Check file permissions (Windows/Linux compatible)
            try:
                with open(GOOGLE_CREDENTIALS_PATH, 'r') as f:
                    f.read(1)  # Test read access
            except PermissionError:
                print(f"❌ Permission denied reading: {GOOGLE_CREDENTIALS_PATH}")
                print(f"   💡 Right-click credentials.json → Properties → Security → Ensure read access")
                return None
            ...
            # Authorize client (gspread 5.x compatible)
            ...
            try:
                ...
                print(f"   💡 SHARE the sheet with: slci-sheets-bot@slci-chatbot.iam.gserviceaccount.com")
                print(f"   💡 Give 'Editor' permissions")
                gs_client = None
                return None
...
def _get_sheet_headers(sheet_name):
    ...
    headers_map = {
        "Downloads": [
            "Timestamp", "Full Name", "Company Name", "Email", "Contact Number",
            "Designation", "Rating", "State", "Act Type", "IP Address", "Download ID"
        ],
        "Service_Enquiries": [
            "Timestamp", "Enquiry ID", "Full Name", "Company Name", "Email",
            "Contact Number", "Service", "Query", "IP Address", "Status"
        ],
        "Fee_Enquiries": [
            "Timestamp", "Enquiry ID", "Full Name", "Company Name", "Email",
            "Contact Number", "Description", "IP Address", "Status"
        ]
    }
...
def append_to_google_sheet(sheet_name, data_row):
    ...
    try:
        ...
        try:
            ...
            print(f"   💡 Ensure it's shared with: slci-sheets-bot@slci-chatbot.iam.gserviceaccount.com")
            return False
        ...
        # Normalize data_row keys to lowercase with underscores
        normalized_data = {}
        ...
        # Get current headers from sheet (lowercase normalized)
        sheet_headers = [h.lower().replace(' ', '_') for h in worksheet.row_values(1)]
        
        # Build row values matching sheet headers order
        row_values = []
        ...
        # Append with thread-safe lock and retry
        with gs_lock:
        ...
        print(f"   💡 Run as Administrator or fix file permissions on credentials.json")
        return False
    except Exception as e:
...
SERVICES_DATA = [
    {
        ...
        "description": "SLCI is a specialist Labour Law Compliance firm with over three decades of experience, helping businesses ensure 100% statutory compliance and risk-free operations through audits, SOP guidance, and periodic compliance support. We provide expert assistance across EPF, ESI, Minimum Wages, Gratuity, POSH, Contract Labour, and other key enactments."
    },
    {
        "title": "Labour Law Auditing",
        "description": "SLCI provides comprehensive Auditing & Assurance services to identify statutory compliance gaps, uncover risk exposures, and prevent costly penalties. Our audits include document checklists, critical analysis, detailed reports, and customized risk-elimination strategies."
    },
    {
        "title": "Labour Law Consultation",
        "description": "SLCI offers expert Labour Law Consultancy, delivering practical and long-term solutions for statutory compliance, due diligence, and risk-free business operations. We support startups, growing businesses, and organizations undergoing mergers or expansions across all industries."
    },
    {
        "title": "HR Solution",
        "description": "SLCI provides comprehensive HR Consulting services, helping businesses design compliant, structured, and cost-effective HR policies and systems. We assist with HR, Leave, POSH, and Work From Home policies, CTC structuring, ICC formation, and NDA agreements."
    },
    {
        "title": "ESI & EPF",
        "description": "SLCI provides end-to-end ESI & EPF Compliance services, ensuring accurate contributions, timely filings, and complete statutory adherence. Our support includes employee registration, UAN generation, challans, KYC approvals, inspections, and automated monthly reports."
    },
    {
        "title": "Payroll Compliance",
        "description": "SLCI provides customised Payroll Compliance solutions, ensuring accurate wage structuring, statutory compliance, and error-free payroll processing. Our technology-driven system offers real-time tracking, digital records, and dedicated payroll support."
    },
    {
        "title": "Recruitment",
        "description": "Identifying and hiring the right talent aligned with your company's skills, culture, and business objectives."
    },
    {
        "title": "Background Verification",
        "description": "Verifying candidate credentials, employment history, and records to ensure authenticity and reduce hiring risks."
    },
    {
        "title": "Staffing",
        "description": "Providing skilled manpower solutions to meet short-term, long-term, or project-based workforce requirements."
    }
]
...
ESI_EPF_INFO = {
    ...
    <h4 style="color: #1a237e; margin-bottom: 15px;">ESI and EPF Compliance Services</h4>
    <p>We at SLCI have in-house experts and consultants along with high tech and unique software that ensures 100% compliance to legislations. We holistically guide our clients and educate them about protocols of ESI and EPF with aid of frequent educational webinars.</p>
    <p>We have a fully automated tech-driven system that helps us complete all ESI and EPF compliances well in time. SLCI ensures all its clients are updated with latest amendments and provides a monthly calendar checklist for ease of remembering the important dates.</p>
    </div>
    """,
    "services": [
        "Registration of Employees",
        "Generation of UAN no.",
        "KYC Digital Approvals",
        "Generation of Challans",
        "Other online support services including changes in any employee details",
        "Guidance and Maintenance of Statutory Records and submission of Statutory returns from time to time",
        "To provide support in ESI and provident fund inspections in the completion of Statutory provisions",
        "Fully automated support wherein monthly reports are sent out to our clients"
    ]
}
...
RESPONSES = {
    ...
    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 4px solid #667eea;">
        <h3 style="color: #667eea; margin-top: 0;">📍 Our Office Address</h3>
        <p style="color: #333; font-size: 16px; line-height: 1.6; margin: 15px 0;">
            83, DSIDC COMPLEX,<br>
            Okhla I Rd, Pocket C,<br>
        ...
        <div style="margin: 20px 0; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
            <iframe 
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3502.1234567890123!2d77.2818578!3d28.525507!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x390ce1b166f71281%3A0x78602061339ccb61!2sShakti%20Legal%20Compliance%20India%20(SLCI)!5e0!3m2!1sen!2sin!4v1234567890123!5m2!1sen!2sin" 
                width="100%" 
                height="300" 
                style="border:0;" 
                allowfullscreen="" 
        ...
        <div style="text-align: center; margin-top: 20px;">
            <a href="https://www.google.com/maps/place/Shakti+Legal+Compliance+India+(SLCI)/@28.5253303,77.2827107,18z/data=!3m1!4b1!4m6!3m5!1s0x390ce1b166f71281:0x78602061339ccb61!8m2!3d28.525507!4d77.2818578!16s%2Fg%2F11j0ww1tn5?entry=ttu" 
               target="_blank" 
               style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 12px 30px; text-decoration: none; border-radius: 25px; font-weight: 600; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4); transition: all 0.3s ease;">
               🗺️ Click here to open in Google Maps
            </a>
        </div>
    </div>
    """,
    "timing": """
    <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 4px solid #667eea;">
        <h3 style="color: #667eea; margin-top: 0;">🕒 Office Timing</h3>
        <p style="color: #333; font-size: 16px; line-height: 1.8; margin: 15px 0;">
            <strong>Monday - Saturday:</strong> 9:00 AM to 6:00 PM<br>
            <strong>Sunday:</strong> Closed
        </p>
        <p style="color: #666; font-size: 14px; margin-top: 15px; font-style: italic;">
            * We recommend scheduling appointments in advance for personalized consultation.
        </p>
    </div>
    """,
    "why slci": """
    <div style="background: linear-gradient(135deg, #667eea15 0%, #764ba215 100%); padding: 25px; border-radius: 12px; border: 1px solid #667eea30;">
        <h3 style="color: #667eea; margin-top: 0;">✨ Why Choose SLCI?</h3>
        
        <div style="margin: 20px 0;">
            <p style="color: #333; font-size: 18px; font-weight: 600; margin: 10px 0;">
                🏆 38+ Years of Experience in the Field of Law
            </p>
            <p style="color: #555; font-size: 15px; line-height: 1.7; margin: 15px 0;">
                We help clients achieve their goals by providing <strong>high-quality, ethically sound legal counsel</strong> 
                and <strong>strategic advice</strong> tailored to your business needs.
            </p>
        </div>
        
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin: 20px 0;">
            <div style="background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <strong style="color: #667eea;">✅</strong> Expert Legal Team
            </div>
            <div style="background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <strong style="color: #667eea;">✅</strong> Pan-India Coverage
            </div>
            <div style="background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <strong style="color: #667eea;">✅</strong> 24/7 Support
            </div>
            <div style="background: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,0.08);">
                <strong style="color: #667eea;">✅</strong> Cost-Effective Solutions
            </div>
        </div>
        
        <p style="color: #667eea; font-weight: 500; margin-top: 20px; text-align: center;">
            🤝 Your Compliance Partner for Growth & Peace of Mind
        </p>
    </div>
    """,
    ...
    "pricing": """
    Our service fees are customized based on your business size, industry, and compliance requirements.
    Please click the button below to submit your enquiry.
    Our team will provide a detailed quotation within 24 hours.
    """,
...
def log_download_request(data, ip_address, user_agent):
    """Log download to DB AND Google Sheets - Fixed"""
    conn = None
    cursor = None
    try:
        conn = get_db_connection()
        ...
        (full_name, company_name, email, contact_number, designation, rating,
        state, act_type, ip_address, user_agent)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ''', (
            data['fullName'], data['companyName'], data['email'],
            data['contactNumber'], data['designation'], int(data['rating']),
            data['state'], data['actType'], ip_address, user_agent
        ))
        ...
        ON CONFLICT(state, act_type) DO UPDATE SET
        download_count = download_stats.download_count + 1,
        ...
        # ALSO save to Google Sheets (non-blocking background thread)
        try:
            sheet_data = {
...
# ============================================================================
# IMPROVED STATE DETECTION FUNCTION
# ============================================================================
def detect_state(message, act_type="minimum_wages"):
    """Improved state detection with better matching for all act types"""
    message_lower = message.lower()
    
    # First, try to extract state using common patterns
    state_patterns = [
        r'(?:of|for|in)\s+([a-zA-Z\s]+?)(?:\?|$|\.)',
        r'([a-zA-Z\s]+?)\s+(?:state|act|rules|law)',
        r'what is .*? (?:act|rules|law) (?:of|for|in) ([a-zA-Z\s]+)',
    ]
    ...
    if act_type == "minimum_wages":
        ...
        # First check direct state names
...
def request_download():
    ...
    try:
        ...
        # Log the download request to DB FIRST
        ip_address = request.remote_addr
        user_agent = request.headers.get('User-Agent')
        download_id = log_download_request(data, ip_address, user_agent)
        
        # Store pending download data temporarily (with 10-min expiry)
        with pending_lock:
        ...
        # Return JSON with token (NOT the PDF yet)
        return jsonify({
            "success": True,
...
def generate_pdf(token):
    ...
    try:
        ...
        with pending_lock:
            ...
            # Clean up token after use (one-time download)
            del pending_downloads[token]
        ...
        # Fetch content based on act_type
        pdf_data = None
        if act_type == 'minimum_wages':
        ...
        pdf_file = create_pdf_file(
            state,
            pdf_data.get("act_type", act_type),
            pdf_data.get("tables_data", []),
            pdf_data.get("effective_date"),
            download_id
        )
...
def create_pdf_file(state, act_type, tables_data, effective_date, download_id=None):
    ...
    # PROFESSIONAL HEADER (Same for all PDFs)
    ...
    # EFFECTIVE DATE (Only show if available)
    if effective_date:
        elements.append(Paragraph(f"<b>Effective Date: {effective_date}</b>", date_style))
    
    elements.append(Spacer(1, 10))
    
    # TABLES (Consistent styling for all act types)
    if tables_data:
    ...
    # CLEAN FOOTER (Same for all PDFs)
...
def extract_effective_date(soup):
    date_patterns = [
        r'Effective from Date:\s*(\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+,?\s+\d{4})',
        r'📋 Effective from Date:\s*(\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+,?\s+\d{4})',
        r'Effective[:\s]+from[:\s]+Date[:\s]*(\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+,?\s+\d{4})',
        r'w\.e\.f[.\s]*(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})',
        r'Effective Date[:\s]*(\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+,?\s+\d{4})',
    ]
    ...
    try:
        ...
        <div class="effective-date-banner">
            <div class="date-content">
                ...
                <span class="date-value">{effective_date or 'Check on website'}</span>
            </div>
        </div>
        ''' if effective_date else '''
        <div class="effective-date-banner warning">
            <div class="date-content">
                <i class="fas fa-exclamation-triangle"></i>
                <span class="date-label">EFFECTIVE DATE:</span>
                <span class="date-value">Check on website</span>
            </div>
        </div>
        '''
        ...
        <div style="margin: 20px 0; text-align: right;">
            <button onclick="openDownloadModal('{state_url_encoded}', 'minimum_wages')" class="download-pdf-btn">
        ...
        final_html = f"""
        <div>
            <h3 style="margin-bottom:15px;">Minimum Wages – {state.title()}</h3>
            {date_header}
            {download_button}
            {html_output}
            <div style="margin-top:20px; font-size:12px; color:#666; text-align:center; border-top:1px solid #ccc; padding-top:15px;">
                Source: <a href="{url}" target="_blank">slci.in/minimum-wages/{state}</a>
            </div>
        </div>
        """
        return {
            ...
            "state": state,
            "act_type": "Minimum_Wages"
        }
    except Exception as e:
        print(f"Error fetching wages for {state}: {str(e)}")
        return {
            "html": f"<p>Error fetching wages data for {state.title()}.</p>",
            "effective_date": None,
            "tables_data": []
        }
...
def fetch_holiday_list(state):
    try:
        ...
        <div style="text-align:right; margin:15px 0;">
            <button onclick="openDownloadModal('{download_state}', 'holiday_list')" class="download-pdf-btn">
        ...
        final_html = f"""
        <div>
            <h3>Holiday List – {state.title()}</h3>
            {download_button}
            {html_output}
...
def fetch_working_hours(state):
    try:
        ...
        download_button = f'''
        <div style="text-align:right; margin:15px 0;">
            <button onclick="openDownloadModal('{state_encoded}', 'working_hours')" class="download-pdf-btn">
                <i class="fas fa-file-pdf"></i> Download Working Hours PDF
            </button>
        </div>
        '''
        ...
        <div class="effective-date-banner">
            <div class="date-content">
                <i class="fas fa-calendar-check"></i>
                <span class="date-label">EFFECTIVE DATE:</span>
                <span class="date-value">{effective_date}</span>
            </div>
        </div>
        ''' if effective_date else '''
        <div class="effective-date-banner warning">
        ...
        final_html = f"""
        <div>
            <h3>Working Hours – {state.title()}</h3>
            {date_header}
            {download_button}
            {html_output}
            <div style="margin-top:20px; font-size:12px; color:#666; text-align:center; border-top:1px solid #ccc; padding-top:15px;">
...
# ============================================================================
# IMPROVED SHOP ESTABLISHMENT FETCH FUNCTION
# ============================================================================
def fetch_shop_establishment(state):
    ...
    try:
        ...
        # If state is None or "all_states", show the full list
        if state is None or state.lower() == "all_states":
            ...
            return {
                ...
                "tables_data": [],
                "state": "All States",
                "act_type": "Shop_and_Establishment",  
                "effective_date": None
            }
        
        # Filter tables for the specific state
        filtered_tables = []
        state_lower = state.lower()
        if tables_data:
            for table in tables_data:
                # Check if any cell in the table contains the state name
                state_found = False
                filtered_rows = []
                for row in table:
                    # Keep header row always
                    if row == table[0]:
                        filtered_rows.append(row)
                        continue
                    # Check if this row belongs to the requested state
                    ...
                    if state_lower in row_text:
                        ...
                        for variation in STATE_VARIATIONS.get(state_lower, []):
                            if variation in row_text:
                                ...
                                filtered_rows.append(row)
                                break
                if state_found and len(filtered_rows) > 1:  # Has at least one data row
                    filtered_tables.append(filtered_rows)
        
        # If no filtered data found, show a message
        if not filtered_tables:
            html_output = f"""
            <div style="padding: 20px; text-align: center; background: #fff3cd; border-radius: 8px;">
                <i class="fas fa-info-circle" style="color: #856404;"></i>
                <p style="color: #856404; margin-top: 10px;">
                    No specific Shop & Establishment data found for <strong>{state.title()}</strong>.<br>
                    Please check our main page for more details.
                </p>
            </div>
            """
        else:
            # Build HTML for filtered tables
        ...
        return {
            "html": final_html,
            "tables_data": filtered_tables if filtered_tables else tables_data,
            "state": state,
            "act_type": "Shop_and_Establishment",
            "effective_date": None
        }
        ...
        return {
            ...
            <div style="padding: 20px; text-align: center; background: #f8d7da; border-radius: 8px;">
                ...
                <p style="color: #721c24; margin-top: 10px;">
                    Error fetching Shop & Establishment data for {state.title()}.<br>
...
def get_fast_response(query):
    try:
        ...
        payload = {
            ...
            "prompt": prompt,
            "stream": False,
            "options": {"temperature": 0.3, "max_tokens": 100, "num_predict": 100}
        }
...
def send_service_enquiry_email(data, enquiry_id):
    ...
    try:
        ...
        .enquiry-id {{ background: #667eea; color: white; padding: 10px; text-align: center; font-size: 18px; }}
        </style>
        </head>
        <body>
        <div class="container">
            ...
            <div class="content">
                ...
                <div class="field">
                    ...
                    <div class="value">{datetime.now().strftime('%d %B %Y at %I:%M %p')}</div>
                </div>
            </div>
            <div class="footer">
                <p>This is an automated message from SLCI Chatbot System</p>
                <p>© {datetime.now().year} Shakti Legal Compliance India</p>
            </div>
        </div>
        </body>
        </html>
        ...
        server = smtplib.SMTP(os.getenv('EMAIL_HOST', 'smtp.gmail.com'),
...
def send_fee_enquiry_email(data, enquiry_id):
    ...
    try:
        ...
        .header {{ background: linear-gradient(135deg, #1a237e 0%, #283593 100%); color: white; padding: 20px; text-align: center; }}
        .content {{ padding: 20px; background: #f9f9f9; }}
        .field {{ margin: 15px 0; padding: 10px; background: white; border-left: 4px solid #ff9800; }}
        .label {{ font-weight: bold; color: #1a237e; }}
        .value {{ margin-top: 5px; }}
        .footer {{ text-align: center; padding: 20px; color: #666; font-size: 12px; }}
        .enquiry-id {{ background: #ff9800; color: white; padding: 10px; text-align: center; font-size: 18px; }}
        </style>
        </head>
        <body>
        <div class="container">
            <div class="header">
                <h2>💰 New Fee Enquiry Received</h2>
            </div>
            <div class="enquiry-id">
                Enquiry ID: {enquiry_id}
            </div>
...
# ============================================================================
# IMPROVED CHAT ROUTE WITH BETTER STATE HANDLING
# ============================================================================
...
def chat():
    ...
    # 1. SHOP & ESTABLISHMENT QUERIES
    if "shop and establishment" in user_message or \
       "shop establishment" in user_message or \
       "shop & establishment" in user_message or \
       "sea act" in user_message:
        # Try to detect state from the message
        state, _ = detect_state(user_message, "shop_establishment")
        # If state detected, show ONLY that state's data
        if state:
            se_data = fetch_shop_establishment(state)
            return jsonify({
                "response": se_data["html"],
                "state": state,
                "act_type": "shop_establishment"
            })
        # Check if user explicitly asks for all states
        ...
        # If no state detected, show common states as suggestions
        ...
        return jsonify({
            ...
            <div style="padding:15px;">
                <h4 style="color:#1a237e;">Please specify a state</h4>
                <p>Example: "Shop & Establishment Act of Delhi"</p>
                <p>Common states:</p>
    ...
    if "holiday" in user_message:
        ...
        if state:
            ...
            if holiday_data:
                return jsonify({
                    "response": holiday_data["html"],
                    "state": state,
                    "act_type": "holiday_list"
                })
        # If no state detected, show suggestions
        common_states = ["Delhi", "Maharashtra", "Karnataka", "Tamil Nadu", "West Bengal"]
        suggestions = "".join([f"<li style='padding:5px;'>📅 {state}</li>" for state in common_states])
        return jsonify({
            "response": f"""
            <div style="padding:15px;">
                <h4 style="color:#1a237e;">Please specify a state for Holiday List</h4>
                <p>Example: "Holiday list of Maharashtra"</p>
                <p>Common states:</p>
                <ul style="list-style-type:none; padding:0;">
                {suggestions}
                </ul>
    ...
    if "working hours" in user_message or "working hour" in user_message:
        ...
        return jsonify({
            "response": f"""
            <div style="padding:15px;">
                <h4 style="color:#1a237e;">Please specify a state for Working Hours</h4>
                <p>Example: "Working hours of Delhi"</p>
                <p>Common states:</p>
                <ul style="list-style-type:none; padding:0;">
                {suggestions}
                </ul>
    ...
    if "minimum wage" in user_message or "minimum wages" in user_message:
        ...
        return jsonify({
            "response": f"""
            <div style="padding:15px;">
                <h4 style="color:#1a237e;">Please specify a state for Minimum Wages</h4>
                <p>Example: "Minimum wages of Delhi"</p>
                <p>Common states:</p>
                <ul style="list-style-type:none; padding:0;">
                {suggestions}
                </ul>
    ...
    # 1.5 SERVICES LIST QUERY - UPDATED WITH DESCRIPTIONS
    # Fixed: Removed empty string "" from keywords to prevent matching everything
    ...
    if any(phrase in user_message for phrase in service_keywords):
        services_html = """
        <div style="font-family: Arial, sans-serif; padding: 15px; background: linear-gradient(135deg, #f5f7fa 0%, #e9ecef 100%); border-radius: 10px; max-height: 500px; overflow-y: auto;">
            <h4 style="color: #1a237e; margin-bottom: 15px; display: flex; align-items: center; gap: 8px;">
                <i class="fas fa-briefcase" style="color: #667eea;"></i> Our Services
            </h4>
            <div style="display: grid; grid-template-columns: 1fr; gap: 15px;">
        """
        for service in SERVICES_DATA:
            services_html += f"""
            <div style="background: white; padding: 15px; border-radius: 8px; border-left: 4px solid #667eea; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
                <div style="display: flex; align-items: flex-start; gap: 10px; flex-direction: column;">
                    <h5 style="margin: 0; color: #1a237e; font-size: 16px;">{service['title']}</h5>
                    <p style="margin: 5px 0 0; color: #555; font-size: 14px; line-height: 1.5;">{service['description']}</p>
                    <button onclick="openServiceModal('{service['title']}')"
                    style="background: #667eea; color: white; border: none; padding: 8px 15px; border-radius: 5px; cursor: pointer; font-size: 13px; transition: all 0.3s; align-self: flex-start; margin-top: 10px;">
        ...
        services_html += """
            ...
            <p style="margin-top: 15px; color: #666; font-size: 12px; text-align: center;">
                Click "Enquire" button next to any service to get detailed information
            </p>
        </div>
        """
        return jsonify({
            "response": services_html,
            "show_services": True
        })
    
    # 5. PREDEFINED RESPONSES USING KEYWORDS
    for key, keywords in KEYWORDS.items():
        for keyword in keywords:
            if keyword in user_message:
                response_text = RESPONSES.get(key, "")
                # Special handling for pricing queries - show fee enquiry button
                if key in ["pricing", "fees", "cost"]:
                    return jsonify({
                        "response": response_text,
                        "show_fee_button": True
                    })
                # Special handling for ESI/EPF info
                if key in ["epf", "esi"]:
                    enriched_response = f"""
                    <div style="font-family: Arial, sans-serif;">
                        <p>{response_text}</p>
                        <div style="margin-top: 15px; background: #f5f7fa; padding: 15px; border-radius: 8px;">
                            <h4 style="color: #1a237e;">Related Services:</h4>
                            <ul style="list-style-type: none; padding: 0;">
                                <li style="margin: 5px 0;">✅ Registration of Employees</li>
                                <li style="margin: 5px 0;">✅ Generation of Challans</li>
                                <li style="margin: 5px 0;">✅ Monthly Compliance Reports</li>
                            </ul>
                        </div>
                    </div>
                    """
    ...
    # 6. TRY OLLAMA FOR UNKNOWN QUERIES (if available)
    ...
    return jsonify({
        ...
        <div style="padding: 15px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #0d6efd;">
            <p style="margin: 0; color: #333;">
                Thank you for contacting <strong>Shakti Legal Compliance India</strong>.
            </p>
            <p style="margin-top: 8px; color: #555;">
                For detailed assistance regarding your query, please contact our team.
            </p>
            <div style="margin-top: 10px; color: #444;">
                📞 <strong>Phone:</strong> +91 9999329153<br>
                📧 <strong>Email:</strong> info@slci-india.com
            </div>
        </div>
        """
    })
...
def submit_service_enquiry():
    ...
    try:
        ...
        if email_sent:
            ...
            (enquiry_id, full_name, company_name, email, contact_number, service, query, ip_address)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            ''', (
                enquiry_id, data['fullName'], data['companyName'],
                data['email'], data['contactNumber'], data['service'],
                data['query'], request.remote_addr
            ))
            conn.commit()
...
def generate_enquiry_id(prefix="ENQ"):
    ...
    Example: ENQ-20260220-AB12CD
    """
    date_part = datetime.now().strftime("%Y%m%d")
    random_part = secrets.token_hex(3).upper()  # 6-char random
...
def submit_fee_enquiry():
    ...
    try:
        ...
        if email_sent:
            ...
            (enquiry_id, full_name, company_name, email, contact_number, description, ip_address)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            ''', (
                enquiry_id, data['fullName'], data['companyName'],
                data['email'], data['contactNumber'], data['description'],
                request.remote_addr
            ))
            conn.commit()
...
def download_data(state, act_type):
    try:
        ...
        if act_type == 'holiday_list':
            ...
            pdf_file = create_pdf_file(state_key, holiday_data.get("act_type", act_type),
                                       holiday_data.get("tables_data", []),
                                       holiday_data.get("effective_date"), download_id)
            ...
            pdf_file = create_pdf_file(matched_state, act_data.get("act_type", act_type),
                                       act_data.get("tables_data", []),
                                       act_data.get("effective_date"), download_id)
            ...
            if not wh_data:
                return jsonify({"error": "No data available"}), 404
            pdf_file = create_pdf_file(matched_state, wh_data.get("act_type", act_type),
                                       wh_data.get("tables_data", []),
                                       wh_data.get("effective_date"), download_id)
            filename = f"Working_Hours_{matched_state.replace(' ', '_')}.pdf"
            return send_file(pdf_file, mimetype='application/pdf', as_attachment=True, download_name=filename)
        elif act_type == 'shop_establishment':
            matched_state = None
            # Check if state is in variations
            for key in STATE_VARIATIONS.keys():
                if key == state_key or key.replace(' ', '_') == state:
                    matched_state = key
                    break
            # If not found in variations, use state_key directly if it looks valid
            if not matched_state:
                matched_state = state_key
            se_data = fetch_shop_establishment(matched_state)
            if not se_data:
                return jsonify({"error": "No data available"}), 404
            pdf_file = create_pdf_file(matched_state, se_data.get("act_type", act_type),
                                       se_data.get("tables_data", []),
                                       se_data.get("effective_date"), download_id)
...
def check_ollama_status():
    ...
    return jsonify({
        ...
        "model": OLLAMA_MODEL if is_running else None
    })
